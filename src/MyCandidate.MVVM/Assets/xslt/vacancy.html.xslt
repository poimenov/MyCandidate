...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="/">
    <html>
      <head>
        <title><xsl:value-of select="ext:GetLocalized('Vacancy')"/>: <xsl:value-of select="Vacancy/@title"/></title>
        <style>
        table {
          width: 100%;
    ...
    <body>
      <h2 style="text-align: center;"><xsl:value-of select="ext:GetLocalized('Vacancy')"/>: <xsl:value-of select="@title"/></h2>
      <table>
        <colgroup>
        ...
        <tr>
          <td>
            <table>
              ...
              <tr>
                <td class="lbl"><xsl:value-of select="ext:GetLocalized('Description')"/></td>
                <td class="txt">
          ...
          <td>
            <table>
              ...
              <tr>
                <td class="lbl"><xsl:value-of select="ext:GetLocalized('Company_Name')"/></td>
                <td class="txt"><xsl:value-of select="Company/@name"/></td>
              </tr> 
              <tr>
                <td class="lbl"><xsl:value-of select="ext:GetLocalized('Office_Name')"/></td>
                <td class="txt"><xsl:value-of select="Company/Office/@name"/></td>
              </tr>  
              <tr>
                <td class="lbl"><xsl:value-of select="ext:GetLocalized('Address')"/></td>
                <td class="txt"><xsl:apply-templates select="Company/Office/Location"/></td>
              </tr>                
              <tr>
                <td class="lbl"><xsl:value-of select="ext:GetLocalized('Vacancy_Status')"/></td>
                <td class="txt"><xsl:value-of select="VacancyStatus/@name"/></td>
              </tr> 
              <tr>
        ...
        <tr>
          <td colspan="2">
            <fieldset>
              <legend><xsl:value-of select="ext:GetLocalized('Candidates')"/></legend>
              <div style="overflow-x: auto;">
                <table class="candidates">
                <tr style="background-color: Gainsboro;">
                  <!-- <th><xsl:value-of select="ext:GetLocalized('Title')"/></th> -->
                  <th><xsl:value-of select="ext:GetLocalized('Full_Name')"/></th>
                  <th><xsl:value-of select="ext:GetLocalized('Selection_Status')"/></th>                  
                  <th><xsl:value-of select="ext:GetLocalized('Address')"/></th>
...
<xsl:template match="CandidateOnVacancy">
  <tr>
    <!-- <td><xsl:value-of select="Candidate/@title"/></td> -->
    <td><xsl:value-of select="concat(Candidate/@lastName, ' ', Candidate/@firstName)"/></td>
    <td><xsl:value-of select="@status"/></td>
    <td><xsl:apply-templates select="Candidate/Location"/></td>
    <td><div class="flex-container"><xsl:apply-templates select="Candidate/Resources/Resource"/></div></td>
    <td><div class="flex-container"><xsl:apply-templates select="Candidate/Skills/Skill"/></div></td>
  </tr>
</xsl:template>
